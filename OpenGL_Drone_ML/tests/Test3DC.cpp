...
namespace test
{
          ...
          m_Window(window), m_FreeLookEnabled(false), m_LeftClick(false), m_TargetTranslation(200, 200, 0)
    {
        ...
        layoutScreen.Push<float>(3);
        layoutScreen.Push<float>(3);
        layoutScreen.Push<float>(2);
        layoutScreen.Push<float>(1);
        ...
        PushCube(positionsMapElements, indicesMapElements, 1600.0f, 1.0f, -1500.0f, 1600.0f, 2.0f, 1500.0f, {0.0, 0.0, 0.0}, 1.0f, &m_Terrain);
        /*
        ...
        layoutMap.Push<float>(3);
        layoutMap.Push<float>(3);
        layoutMap.Push<float>(2);
        layoutMap.Push<float>(1);
        ...
        layoutPickupZones.Push<float>(3);
        layoutPickupZones.Push<float>(3);
        layoutPickupZones.Push<float>(2);
        layoutPickupZones.Push<float>(1);
        ...
        layoutDrone.Push<float>(3);
        layoutDrone.Push<float>(3);
        layoutDrone.Push<float>(2);
        layoutDrone.Push<float>(1);
    ...
    void Test3DC::OnUpdate(float deltaTime)
    {
        ...
        else
        {
            ...
            if (std::abs(m_Drone.x - m_TargetTranslation.x) < 1.0f && std::abs(m_Drone.y - m_TargetTranslation.y) < 1.0f && std::abs(m_Drone.z - m_TargetTranslation.z) < 1.0f)
            {
                // Snap to target
    ...
    void Test3DC::OnImGuiRender()
    {
        ImGuiIO &io = ImGui::GetIO();
        (void)io;
        ImGui::SliderFloat3("m_Drone", &m_Drone.x, -1000.0f, 1000.0f);
    ...
    nlohmann::json Test3DC::BuildPayload()
    {
        nlohmann::json payload;
        if (first_loop)
        {
            payload["test"] = "3DC"; // <-- identifier, tells server what to do!
    ...
    std::vector<std::vector<float>> Test3DC::LidarScanBelow()
    {
    ...
    bool Test3DC::RayIntersectsTriangle(const glm::vec3 &orig, const glm::vec3 &dir, const Triangle &tri, float &outT)
    {
    ...
    void Test3DC::ProcessInput(float deltaTime)
    {
    ...
    void Test3DC::KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
    {
        ImGui_ImplGlfw_KeyCallback(window, key, scancode, action, mods);
        // Grab the instance pointer back out
        auto *self = static_cast<Test3DC *>(glfwGetWindowUserPointer(window));
        if (!self)
            return;
        ...
        if (key == GLFW_KEY_SPACE && action == GLFW_RELEASE)
        {
            ...
            {
                self->m_ServerThread = std::thread(&Test3DC::ServerThreadFunc, self);
                self->m_MakeThread = false; // only make one thread
            }
        }
    }

    void Test3DC::MouseCallback(GLFWwindow *window, double xposIn, double yposIn)
    {
    ...
    void Test3DC::MouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
    {
    ...
    void Test3DC::ScrollCallback(GLFWwindow *window, double xoffset, double yoffset)
    {
        ...
        auto ProcessMesh = [&](aiMesh *mesh)
        {
            ...
            for (unsigned int i = 0; i < mesh->mNumFaces; ++i)
            {
                ...
                if (terrain)
                {
                    ...
                    terrain->push_back({glm::vec3(v0.x, v0.y, v0.z),
